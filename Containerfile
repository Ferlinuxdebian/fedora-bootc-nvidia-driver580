...
RUN KERNEL_VERSION="$(rpm -q kernel-core --queryformat '%{VERSION}-%{RELEASE}.%{ARCH}')" && \
    dnf5 -y --refresh install "kernel-devel-${KERNEL_VERSION}" wget && \
    wget -O /etc/yum.repos.d/fedora-nvidia-580.repo https://negativo17.org/repos/fedora-nvidia-580.repo && \
    dnf5 -y --refresh install nvidia-driver nvidia-driver-cuda && \
...
RUN kver="$(rpm -q kernel-core --queryformat '%{VERSION}-%{RELEASE}.%{ARCH}')" && \
    dnf5 -y --refresh install --setopt=tsflags=nodocs "kernel-modules-extra-${kver}" && \
    ...
    grep -v '^#' /tmp/sysconfig/nvidia_packages | tr '\n' ' ' | xargs dnf5 -y --refresh install --setopt=tsflags=nodocs && \
    dnf5 -y --refresh install /tmp/nvidia/kmod-nvidia-*.rpm && \
    rm -rf /tmp/nvidia
